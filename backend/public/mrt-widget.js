(function () {
  ...
  const css = `
    ...
    .mrt-dot-ok { background-color: #22c55e; box-shadow: 0 0 8px #22c55e; }
    .mrt-dot-warning { background-color: #eab308; box-shadow: 0 0 8px #eab308; }
    .mrt-dot-critical { background-color: #ef4444; box-shadow: 0 0 8px #ef4444; }
  ...
  function renderGlobal(data) {
    return `
      <div class="mrt-w-container" style="display:inline-block;">
  ...
  function renderSummary(data) {
    return `
      <div class="mrt-w-container">
        <h4 style="margin:0 0 8px 0;font-size:14px;font-weight:700;">Estado MRT</h4>
        <div style="font-size:12px;margin-bottom:8px;">
          <div>🟢 Operativos: ${data.operativos}</div>
          <div>🟡 Parciales: ${data.parciales}</div>
          <div>🔴 Caídos: ${data.caidos}</div>
        </div>
        <div style="font-size:10px;color:#64748b;">Act: ${new Date(data.ultimaActualizacion).toLocaleTimeString()}</div>
      </div>
    `;
  }
  ...
  function renderSites(data) {
    const items = data.map(site => `
      <div class="mrt-site-item" data-site-name="${site.sitio}">
        <span class="mrt-dot ${getDotClass(site.estado)}"></span>
        <span>${site.sitio}</span>
        <div class="mrt-tooltip">
          ...
          Disponibilidad: ${site.disponibilidad || 100}%<br/>
    ...
    return `
      <div class="mrt-w-container">
        <h4 style="margin:0 0 8px 0;font-size:13px;font-weight:700;">Lista de Sitios</h4>
    ...
    const items = sites.map(site => `
      <div class="mrt-site-item" data-site-name="${site.sitio}" style="padding:4px 8px;font-size:11px;">
        ...
        <div class="mrt-tooltip">
          <strong>${site.sitio}</strong><br/>
          Estado: ${site.estado.toUpperCase()}<br/>
          Equipos: ${site.equiposActivos}/${site.equiposTotales}
        </div>
      </div>
    `).join("");

    return `
      <div class="mrt-w-container" style="max-width:320px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;border-bottom:1px solid rgba(0,0,0,0.05);padding-bottom:8px;">
          ...
          <span class="mrt-dot ${getDotClass(global.status)}" style="margin-left:auto;"></span>
        </div>
        <div style="font-size:11px;display:flex;gap:12px;margin-bottom:10px;opacity:0.8;">
          <span>🟢 ${global.operativos}</span>
          <span>🟡 ${global.parciales}</span>
          <span>🔴 ${global.caidos}</span>
        </div>
  ...
  window.mrtOpenDetails = async (siteName) => {
    ...
    content.innerHTML = "Cargando equipos...";
    ...
    try {
      ...
      if (!site || !site.equipos) {
        content.innerHTML = "No se encontraron detalles para este sitio.";
        return;
      }

      content.innerHTML = site.equipos.map(eq => `
        <div class="mrt-modal-device">
          <div>
            <strong>${eq.nombre}</strong><br/>
            <span style="font-size:10px;color:#64748b;font-family:monospace;">${eq.ip}</span>
          </div>
          <div style="text-align:right;">
            <span style="font-weight:bold;color:${eq.estado === 'ok' ? '#22c55e' : eq.estado === 'inestable' ? '#eab308' : '#ef4444'}">
              ${eq.estado.toUpperCase()}
            </span><br/>
            <span style="font-size:10px;opacity:0.8;">${eq.latencia} ms | ${eq.disponibilidad}%</span>
          </div>
        </div>
      `).join("");
    } catch (e) {
      content.innerHTML = "Error al obtener detalles.";
    }
  };
