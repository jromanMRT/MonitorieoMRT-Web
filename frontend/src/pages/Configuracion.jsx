...
export default function Configuracion({ onConfigChange }) {
  ...
  const cargarEquipos = async () => {
    try {
      const r = await axios.get("/api/equipos-config");
      setEquipos(r.data || []);
  ...
  const handleEquipoSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        // Actualizar equipo
        await axios.put(`/api/equipos-config/${editingId}`, { sitio, nombre, ip });
        setFormSuccess("Equipo actualizado exitosamente.");
        setEditingId(null);
      } else {
        // Crear equipo
        await axios.post("/api/equipos-config", { sitio, nombre, ip });
  ...
  const eliminarEquipo = async (id, nombre) => {
    ...
    try {
      await axios.delete(`/api/equipos-config/${id}`);
  ...
  const toggleEquipo = async (eq) => {
    try {
      await axios.patch(`/api/equipos-config/${eq.id}/toggle`);
